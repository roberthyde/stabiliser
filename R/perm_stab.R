...
perm_stab <- function(data, boot_data, perm_data, outcome, boot_reps, permutations, perm_boot_reps, model_name, type, quantile) {
  try({
    ...
    model_time = as.numeric(difftime(end_time,start_time, units = "secs"))

    if(model_time > 60){
      model_time <- paste(round(model_time/60/60,2), "hours")
    }
    else{
      model_time <- paste(round(model_time,2), "seconds")
    }
