...
stabilise_re_glmer <- function(data, outcome, intercept_level_ids, n_top_filter = 50,
                               ...
                               normalise = TRUE, dummy = TRUE, impute = TRUE, parallel = TRUE, cores = 5, force_vars = NULL) {

  if(parallel == TRUE){
    plan(multisession, workers = cores)
    message("Parallel processing set up with ", cores, " cores")
  }
  ...
  ## Prep force vars, e.g. for survival model
  if(!is.null(force_vars)== TRUE){
    force_vars_plus <- paste0(force_vars, " + ", sep = "")
  } else{
    force_vars_plus <- ""
  } ## So that binds an emplty placeholder into the model formula if there are no force vars
  ...
  ## Set up DF to catch coefficients - including factor levels if factors are present
  x_names <- data %>%
  ...
  boot_function <- function(i, data_selected){

    ## bootstrap to keep equal proportions of outcome across bootstrap samples (to prevent failure with low 1:0 proportion)
    RE_boot <- data_selected %>%
      group_by(across(all_of(outcome))) %>%                     # group by outcome
      slice_sample(prop = 1, replace = TRUE) %>% # sample within each group
      ungroup()
    ...
    print(mod_code)
    mod_sim_RE_boot <- suppressMessages(glmer(paste0(outcome, " ~ ", force_vars_plus, mod_code, rand_names), data = RE_boot, family = binomial(link= "logit"), control=glmerControl(optimizer="nlminbwrap")))
    ...
    if(ncol(boot_final_mod_data_2) > length_rand){
      ...
      final_boot_mod <- suppressMessages(glmer(paste0(outcome, " ~ ",  final_mod_code, rand_names), data = boot_final_mod_data_2, family = binomial(link= "logit"), control=glmerControl(optimizer="nlminbwrap")))
    }
    ...
    if(ncol(boot_final_mod_data_2) <= length_rand){  # TODO: Can't rely on implied column numbers here - this is because it depends how many random effects there are
      ...
      final_boot_mod <- suppressMessages(glmer(paste0(outcome, " ~ ", force_vars_plus, rand_names_re_only), data = boot_final_mod_data_2, family = binomial(link= "logit"), control=glmerControl(optimizer="nlminbwrap"))) ## Added bracket here CHECK
    }
  ...
  perm_function <- function(i, data, outcome){
  ...
  selected_mod <- glmer(paste0(outcome, " ~ ", force_vars_plus, mod_code_sel, rand_names), data = selected_to__model2, family = binomial(link= "logit"), control=glmerControl(optimizer="nlminbwrap"))
